...
export default function ChatInterfaceOld() {
...
function MessageBubble({ message }: { message: MessageType }) {
  ...
  return (
    <div className={`flex ${isUser ? "justify-end" : "justify-start flex-col md:flex-row"}`}>
      ...
      <div
        className={`w-full mb-10 rounded-[1rem] font-medium ${
          isUser
      ...
      >
        {isUser ? (
          <p className="text-[16px] leading-snug whitespace-pre-wrap">{displayedIntroText}</p>
        ) : ( 
          <div>
            {/* Intro Text - FIXED: Only show animated text, not the full message */}
            <div className="p-4">
              <p className="text-[16px] leading-snug whitespace-pre-wrap">
                {displayedIntroText}
                {!introComplete && (
